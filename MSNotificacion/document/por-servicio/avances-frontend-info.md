# Avances — frontend-info

> Detalle de sesiones en `frontend-info/`. Resumen consolidado: [../avances.md](../avances.md) (**Origen:** FE).
...
#### Completado
- Rutas y UI logística bajo `/dashboard/logistica/*` (ver `frontend-info/document/avances.md` para listado completo).
- `LogisticsLayout`, servicios/hooks, permisos RBAC, mocks KPI/listados.
- Shell dashboard: `dashboard/layout.tsx` + `DashboardThemeProvider` + sidebar único.
- Menú: Gestión Usuarios, Emergencias, Centros, Logística (submenú), Inventario, Gestión Ciudadana (submenú).
- Estilos `sidebar--app`; fix Google Maps loader y dibujo zonas.

#### Próximos pasos
- Páginas usuarios y ciudadana; APIs `GET` listado logística; E2E gateway.
