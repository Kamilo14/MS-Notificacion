...
### [ARR-014-FRONTEND-INTEGRATION] Nota: cambio en flujo de registro desde frontend
- **Fecha:** 2026-05-06
- **Autor:** Camilo / Claude
- **Tipo:** Integración
- **Descripción del cambio:** Durante integración frontend-backend se detectó `EMAIL_EXISTS` al invocar `POST /auth/register` desde la UI. El frontend fue modificado para crear la cuenta en Firebase (cliente), obtener `idToken` y llamar `POST /auth/firebase/sync` para sincronizar el perfil en `ms-identity`, evitando así la creación duplicada.
- **Archivos afectados (frontend):** `frontend-info/src/app/register/page.tsx`, `frontend-info/src/services/auth.service.ts`

### [ARR-013] Centralización de configuración CORS en CorsConfig.java (MS Gateway)
- **Fecha:** 2026-05-06
- **Autor:** Claude
- **Tipo:** Config | Refactor
- **Error relacionado:** N/A
- **Descripción del cambio:** Se creó `CorsConfig.java` en `MS-Api-Gateway/src/main/java/cl/catastrofescl/gateway/config/` con propiedades configurables: `gateway.cors.enabled` (default: true) y `GATEWAY_CORS_ENABLED` en `.env`.
...
### [ARR-012] Implementación de API Gateway (Fase 0.5)
- **Fecha:** 2026-05-05
- **Autor:** Claude
- **Tipo:** Feature
- **Error relacionado:** N/A
- **Descripción del cambio:** Se implementó `ms-gateway` usando Spring Cloud Gateway (reactivo) con validación Firebase Admin SDK, rate limiting con Bucket4j y Headers de Seguridad. Se implementó `GatewayExceptionHandler` para devolver formato RFC 7807.
- **Archivos afectados:** `MS-Api-Gateway/pom.xml`, `MS-Api-Gateway/src/main/resources/application.yml`, `MS-Api-Gateway/src/main/java/.../GatewayApplication.java`, `MS-Api-Gateway/src/main/java/.../filters/*.java`, `.env`
- **Tests actualizados:** N/A

### [ARR-011-IDENTITY] Dockerfile multi-stage y .dockerignore para ms-identity (local)
- **Fecha:** 2026-05-05
- **Autor:** Claude
- **Tipo:** Config
- **Error relacionado:** N/A
- **Descripción del cambio:** Se agregó un `Dockerfile` multi-stage (build con Maven + runtime JRE 21) y `.dockerignore` para construir y ejecutar `ms-identity` en contenedor en entorno local. Se excluyeron explícitamente `.env` y credenciales Firebase del contexto de build para evitar que se horneen secretos en la imagen.
...
- **Tipo:** Feature
- **Error relacionado:** N/A
- **Descripción del cambio:** Se implementó `ms-gateway` usando Spring Cloud Gateway (reactivo). Se eliminaron las dependencias legacy WebMVC e integró validación Firebase Admin SDK (bloqueante manejado con Mono.fromCallable), rate limiting con Bucket4j en memoria y configuración de Headers de Seguridad. Además se implementó un `GatewayExceptionHandler` para devolver siempre formato RFC 7807 (Problem Details). Responsabilidades: validación del token Firebase, enrutamiento a los 6 MS de negocio, rate limiting con Bucket4j, CORS global y headers de seguridad.
- **Archivos afectados:** `MS-Api-Gateway/pom.xml`, `MS-Api-Gateway/src/main/resources/application.yml`, `MS-Api-Gateway/src/main/java/.../GatewayApplication.java`, `MS-Api-Gateway/src/main/java/.../filters/FirebaseAuthenticationFilter.java`, `MS-Api-Gateway/src/main/java/.../filters/RateLimitingFilter.java`, `MS-Api-Gateway/src/main/java/.../filters/SecurityHeadersFilter.java`, `MS-Api-Gateway/src/main/java/.../exception/GatewayExceptionHandler.java`, `.env`
- **Tests actualizados:** N/A

### [ARR-012-FRONTEND-INTEGRATION] Nota: cambio en flujo de registro desde frontend
- **Fecha:** 2026-05-06
- **Autor:** Camilo / Claude
- **Tipo:** Integración
- **Descripción del cambio:** Durante integración frontend-backend se detectó `EMAIL_EXISTS` al invocar `POST /auth/register` desde la UI debido a que el cliente ya había creado la cuenta en Firebase. Para resolverlo, el frontend fue modificado para:
  - Crear la cuenta en Firebase (cliente), obtener `idToken` y llamar `POST /auth/firebase/sync` (endpoint autenticado) para sincronizar el perfil en `ms-identity`.
  - Mostrar al usuario los mensajes detallados del backend (`debugMessage`) en caso de error.
- **Recomendación:** Mantener `POST /auth/register` disponible para clients que prefieran delegar la creación al backend (p.ej. integraciones server-to-server), y considerar en `AuthService` del backend una lógica de recuperación ante `EMAIL_EXISTS` que recupere el UID existente y realice la sincronización si procede.
- **Archivos afectados (frontend):** `frontend-info/src/app/register/page.tsx`, `frontend-info/src/services/auth.service.ts`
- **Archivos afectados (backend):** ninguno modificado ahora; posible mejora: `AuthService` para detectar `EMAIL_EXISTS` y sincronizar si el correo ya existe.
