...
> **SSOT:** `CatastrofeCL-MS/document/plan-de-implementacion.md` — no duplicar en otros `document/`.  
> Progreso vivo: [progreso.md](./progreso.md) | Avances: [avances.md](./avances.md)
...
- [x] Crear entidades JPA en español: `Transferencia`, `ItemTransferencia`, `Mision`, `RutaVoluntario`, `VoluntarioMision` *(2026-06-04, `MSLogistica/`)*
- [x] Migraciones Flyway:
  - [x] `V1__create_transfers_table.sql`
  - [x] `V2__create_transfer_items_table.sql`
  - [x] `V3__create_missions_table.sql`
  - [x] `V4__create_volunteer_routes_table.sql` + índices GIST en coordenadas
  - [x] `V5__create_mission_volunteers_table.sql` + constraint UNIQUE (mision_id, ruta_voluntario_id)
- [x] Implementar `POST /transferencias` — (requiere `TRANSFERENCIA_SOLICITAR`):
  - Validar stock suficiente en centro origen consultando tabla `inventario`
  - Estado inicial: `SOLICITADA`
- [x] Implementar `PATCH /transferencias/:id/estado` — flujo de estados (requiere `TRANSFERENCIA_APROBAR`):
  - `SOLICITADA → APROBADA → EN_TRANSITO → RECIBIDA` (o `RECHAZADA`)
  - En `RECIBIDA`: actualizar inventarios origen y destino `@Transactional`
- [x] Implementar `POST /misiones` — (requiere `MISION_CREAR`)
- [x] Implementar `POST /rutas-voluntario` — voluntario ofrece ruta (requiere `RUTA_OFRECER`)
- [x] Implementar `MatchingOsrmService` — cliente HTTP a OSRM:
  - `GET /rutas-voluntario/matching/:misionId`
  - Filtros: compatibilidad de ruta, capacidad de vehículo, voluntario disponible
  - Ordenamiento: distancia vial OSRM (+ fallback euclidiano si OSRM cae)
- [x] Publicar eventos: `transfer.created`, `transfer.status.changed`
- [ ] Publicar evento: `mission.assigned` *(clase lista; wiring pendiente)*
- [x] Tests de `MatchingOsrmService` con mocks del servidor OSRM
- [x] Tests de integridad transaccional en recepción de transferencia (`TransferenciaServiceTest`)
- [x] Suite unitaria + WebMvcTest (**38** tests, `mvn test`)
- [ ] Servicio `ms-logistics` en `compose.yaml` raíz
- [ ] E2E local vía API Gateway
...
- [x] Crear entidades JPA en español: `Notificacion`, `PreferenciaNotificacion`, `EventoProcesado`
- [x] Migraciones Flyway:
...
- [x] Configurar consumidores `@RabbitListener` para todas las colas de eventos (`notifications.queue` binding `#`)
- [x] Implementar idempotencia: verificar `EventoProcesado` en BD + Redis `processed:{eventId}` TTL 24h
- [x] Implementar `GET /notificaciones` — listado paginado del usuario autenticado
- [x] Implementar `PATCH /notificaciones/:id/leer` — marcar como leída
- [x] Implementar `GET /notificaciones/preferencias` — preferencias del usuario
- [x] Implementar `PATCH /notificaciones/preferencias` — actualizar preferencias
- [x] Configurar servidor WebSocket STOMP:
...
- [x] Crear función Lambda (Node.js) para consumir `email.queue`
- [x] Integrar con AWS SES para envío de correos (modo stub local por defecto)
- [x] Templates de email para: confirmación de donación, stock crítico, transferencia creada/recibida, misión asignada, anuncio urgente
...
- [x] Gestión de centros de acopio e inventarios (rutas FE; centros vía emergencias; inventario placeholder)
...
- [x] Mapa operativo táctico con capas de datos enriquecidas (logística + emergencias)
- [x] Matching OSRM voluntarios (`/dashboard/logistica/matching-osrm`)
- [ ] Publicación de anuncios críticos (formulario con preview)
- [x] Administración de emergencias activas (API real + dibujo zonas por clics)
- [ ] Gestión de usuarios (menú; página `/dashboard/usuarios` pendiente)
