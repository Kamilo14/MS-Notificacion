...
#### Completado
- Se creó `CorsConfig.java` en `src/main/java/cl/catastrofescl/gateway/config/` con configuración CORS centralizada (desde MS-Api-Gateway).
...
### [2026-05-06] Nota: cambio en flujo de registro desde frontend

**Integrante(s):** Claude  
**Fase trabajada:** Fase 1 — Integración Frontend

#### Completado
- Se detectó `EMAIL_EXISTS` al invocar `POST /auth/register` desde la UI.
- El frontend fue modificado para crear la cuenta en Firebase (cliente), obtener `idToken` y llamar `POST /auth/firebase/sync` para sincronizar el perfil en `ms-identity`, evitando la creación duplicada.
- Se implementó formateo automático de RUT (`xx.xxx.xxx-x`) y validación con módulo 11 en el frontend.
- El frontend ahora muestra el `debugMessage` del backend (RFC 7807) para errores claros al usuario.
...
#### Próximos pasos
- Verificar sincronización end-to-end desde UI de registro en local.

---

### [2026-05-05] Contenerización local de ms-identity (Dockerfile)

**Integrante(s):** Claude 
**Fase trabajada:** Fase 0 — Fundamentos e Infraestructura

#### Completado
- Se creó `Dockerfile` multi-stage para construir y ejecutar `ms-identity` (Java 21) exponiendo puerto 8081.
- Se creó `.dockerignore` para excluir `.env` y credenciales Firebase del build context.
...
#### Próximos pasos
- Construir imagen y levantar contenedor junto con PostgreSQL/Redis según `.env` local.
